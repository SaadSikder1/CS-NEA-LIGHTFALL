...
{
    [Header("References")] 
    [SerializeField] private GunData gunData; // Reference to a Asset files (AssaultRIfle.asset) holding weapon stats
    ...
    [SerializeField] private Vector3 aimPos; // Target position when ADS is active
    ...
    private void Start()
    {
        //Capture initial data so we can return to it after recoil or aiming
    ...
    private void Update()
    {
        ...
        // Increment timer by seconds for fire rate calculation
    ...
    private void HandleAiming()
    {
        ...
        // Smoothly transition rotation using Slerp
        transform.localRotation = Quaternion.Slerp(transform.localRotation, targetRot * kickRotation, Time.deltaTime * aimSpeed);

        //Decay the recoil kick back to zero over time smoothhhly so that it doesn't disorient the player 
    ...
    private void Shoot()
    {
        ...
        if (gunData.currentAmmoInMag > 0 && !gunData.isReloading)
        {
            ...
            // Uses Physics Raycasting to detect hits in a straight line from the muzzle
            if (Physics.Raycast(muzzle.position, muzzle.forward, out hit, gunData.maxDistance))
            {
                // see if the object hit can take damage
            ...
            timeSinceLastShot = 0;
            cam.Rotate(-cameraRecoil, 0, 0); // Apply snap
    ...
    private void SpawnBulletTrail(Vector3 hitPoint)
    {
        if (bulletPrefab != null)
        {
            ...
            {
                //Ensures the object is deleted if it has no logic to destroy itself
    ...
    private void Reload()
    {
        ...
        {
            // If reserve is low, just take whatever is left otherrwise it may cfause errors 
    ...
    {
        // Public method to allow pickupsto modify gun state
